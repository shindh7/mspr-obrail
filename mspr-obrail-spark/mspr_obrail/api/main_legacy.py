"""
ObRail Europe - Legacy FastAPI REST API
...
def list_stations(
    ...
    """List stations with coordinates."""
    filters: list[str] = []
    params: list = []
    if country_code:
    ...
    where = f"WHERE {' AND '.join(filters)}" if filters else ""
    sql = f"""
        SELECT station_key, stop_id, station_name, station_lat, station_lon, country_code
        FROM obrail.dim_station
        {where}

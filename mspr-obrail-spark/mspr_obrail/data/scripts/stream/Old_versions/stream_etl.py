...
def _filter_coord_pairs(
    ...
    if missing:
        LOGGER.warning("%s coords ignorées (colonnes manquantes): %s", label, missing)
        return df
    ...
    mask = (
        ...
        & (df[lon_col] != 0)
        & ~(
            (df[lat_col].abs() < COORD_NEAR_ZERO_THRESHOLD)
            & (df[lon_col].abs() < COORD_NEAR_ZERO_THRESHOLD)
        )
    )
    before = len(df)
    df = df[mask].copy()
    removed = before - len(df)
    if removed:
        LOGGER.info("%s supprimées (coords invalides): %s", label, removed)
    return df
...
def _build_service_date_map(
    ...
    calendar_dates_df: pd.DataFrame,) -> dict[str, str]:
...
def get_mobility_database_sources(
    ...
    max_per_country: int = 1,) -> list[dict]:
...
def transform_trip_segments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # FIXED: Deduplicate correctly without dropping valid daily instances
    ...
    if all(col in df.columns for col in ["departure_lat", "departure_lon", "arrival_lat", "arrival_lon"]):
        ...
        distances = distances * DISTANCE_FACTOR
        # FIXED: Saved to "distance_km" and we will NOT drop it below
        df["distance_km"] = distances.round(0).astype("Int64")
        before = len(df)
        df = df[df["distance_km"].notna() & (df["distance_km"] > 0)].copy()
        removed_zero = before - len(df)
        if removed_zero:
            LOGGER.info("Trajets supprimés (distance <= 0km): %s", removed_zero)
        before = len(df)
        df = df[df["distance_km"] >= MIN_TRIP_DISTANCE_KM].copy()
        removed = before - len(df)
        if removed:
    ...
    # Deduplicate symmetric trips (A->B == B->A) within the same operator/country
    pair_cols = ["departure_station", "arrival_station"]
    if all(col in df.columns for col in pair_cols):
        dep = df["departure_station"].astype(str).str.strip().str.lower()
        arr = df["arrival_station"].astype(str).str.strip().str.lower()
        df["_pair_a"] = np.where(dep <= arr, dep, arr)
        df["_pair_b"] = np.where(dep <= arr, arr, dep)
        ...
        removed = before - len(df)
        if removed:
            LOGGER.info("Trajets supprimés (stations inversées): %s", removed)
        df = df.drop(columns=["_pair_a", "_pair_b"], errors="ignore")
    else:
        missing = [col for col in pair_cols if col not in df.columns]
        LOGGER.warning("Dedup trajets inversés ignoré (colonnes manquantes): %s", missing)

    # REMOVED the df.drop(columns=["_distance_km"]) line completely!
...
def _build_dim_station(
    ...
    stations = stations.dropna(subset=["stop_id"])
    
    # FIXED: Group by the natural key to force strict uniqueness and prevent cartesian explosions
    stations = stations.groupby(["stop_id", "country_code"], as_index=False).agg(
        {
            "station_name": "first",
            "station_lat": "first",
            "station_lon": "first",
    ...
    removed = before - len(stations)
    if removed:
        LOGGER.info("Stations supprimées (coords non numériques): %s", removed)

    stations = _filter_coord_pairs(stations, "station_lat", "station_lon", "Stations")

    # Secondary deduplication
    stations = stations.groupby(["stop_id", "country_code"], as_index=False).first()

    stations = stations.reset_index(drop=True)
    stations.insert(0, "station_key", stations.index + 1)
    # Ensure column order matches dim_station schema for COPY ... FROM STDIN
    stations = stations[
        [
            "station_key",
            "stop_id",
...
def _build_fact_segments(
    ...
    # FIXED: Ensure distance_km is actually included before returning the final slice
    if "distance_km" not in fact.columns:
        fact["distance_km"] = pd.NA
...
def _extract_segments_from_zip(content: bytes, country: str, operator: str) -> pd.DataFrame:
    ...
    stops_df = _filter_coord_pairs(stops_df, "stop_lat", "stop_lon", "Stops")
    if stops_df.empty:
        LOGGER.warning("Stops vides après nettoyage: %s (%s).", operator, country)
        return pd.DataFrame()
...
def run_stream_etl(country_codes: set[str] | None = None) -> None:
    ...
    if skip_trip_stops:
        ...
        # FIXED: Added deduplication for trip stops
        trip_stops_df = trip_stops_df.drop_duplicates(
            subset=["trip_id", "date_value", "stop_sequence"]
        ).copy()
    ...
    # Note: Make sure the file paths match your server architecture. 
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
    schema_path = os.path.join(project_root, "data", "scripts", "mart", "schema.sql")

    with _get_conn() as conn:
        with conn.cursor() as cur:
            # Re-run the schema to apply the new column (if handling in-script)
